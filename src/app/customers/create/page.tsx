...
const fetchCustomerTypes = async (): Promise<CustomerType[]> => {
    const response = await GetCustomersTypes();
    return response;
  
};
const fetchPriceLists = async (): Promise<PriceList[]> => {
...
export default function CreateCustomer() {
  ...
  useEffect(() => {
    if (user?.role !== "Vendedor") {
      ...
      // Cargar listas
      Promise.all([fetchSellers(), fetchCustomerTypes(), fetchPriceLists()])
        .then(([sellersData, typesData, pricesData]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createCustomer(dataToSend, user!.token);
